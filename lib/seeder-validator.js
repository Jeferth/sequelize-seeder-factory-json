...
class SeederValidator {
    ...
    static validateSchema(data, schema, entityName = "Entity") {
        ...
        data.forEach((item, index) => {
            ...
            const unexpectedFields = itemFields.filter(field => !schemaFields.includes(field));
        ...
        if (isValid) {
            console.log(`✓ Schema validation passed for ${data.length} ${entityName} records`);
    ...
    static _validateField(value, fieldSchema, fieldName, index, entityName) {
        ...
        if (fieldSchema.required && isValueEmpty) {
            errors.push(`${entityName} item at index ${index}: Field '${fieldName}' is required`);
